...
type DlIdsModel struct {
	...
	remoteURL   string
	onnxReady   bool
	dynSess     *ort.DynamicSession[float32, float32]
	inNames     []string
	outNames    []string
	outShape    []int64
	classLabels []string
	...
	if path != "" && strings.HasSuffix(strings.ToLower(path), ".onnx") {
		...
		if err := ort.InitializeEnvironment(); err == nil {
			...
			if err == nil && len(inInfo) > 0 && len(outInfo) > 0 {
				...
				ds, err := ort.NewDynamicSession[float32, float32](path, inNames, outNames)
				if err == nil {
					m.useRemote = false
					m.onnxReady = true
					m.dynSess = ds
...
func (m *DlIdsModel) Predict(features []float32) (Prediction, error) {
	...
	inTensor, _ := ort.NewTensor[float32](inShape, features)
	outTensor, _ := ort.NewEmptyTensor[float32](ort.NewShape(m.outShape...))
	err := m.dynSess.Run([]*ort.Tensor[float32]{inTensor}, []*ort.Tensor[float32]{outTensor})
	if err != nil {
		return Prediction{Class: "Benign", Confidence: 1.0}, nil
	}
	p := outTensor.GetData()
